	...
	public boolean canContinueToUse() {
		if (super.canContinueToUse()) {
			...
			if (user.getTeam() != null && entity.getTeam() != null) {
				return (user.getTeam().isAlliedTo(entity.getTeam()));
			} else {
				return true;
			}
		}
	...
	public void tick() {
	    if (--this.timeToRecalcPath <= 0)  {
	        ...
	        if (entity.distanceTo(user)>min) {
	        	...
	        	Path path = pather.createPath(user.blockPosition().offset(Math.ceil(dir.x), Math.ceil(dir.y), Math.ceil(dir.z)), 1);
		        if (!pather.moveTo(path, 0.75f)) {
