...
public class DataLoader extends SimpleJsonResourceReloadListener {
	...
	public void apply(Map<ResourceLocation, JsonElement> map, ResourceManager manager, ProfilerFiller profiler) {
		...
		if (!map.containsKey(CONDITIONS)) FollowMe.logError("Failed to load conditions datapack", new FileNotFoundException(CONDITIONS.toString()));
		JsonElement data = map.get(CONDITIONS);
		try {
			if (!data.isJsonObject()) throw new Exception("Conditions datapack has no entries");
			for (Entry<String, JsonElement> entityEntry : data.getAsJsonObject().entrySet()) {
				...
				if (!BuiltInRegistries.ENTITY_TYPE.containsKey(loc)) throw new Exception("Entity " + name + " is not registered");
				EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.get(loc);
				if (!GsonHelper.isValidNode(json, "conditions")) throw new Exception("Entity " + name + " has no defined conditions");
				for (Entry<String, JsonElement> entry : GsonHelper.getAsJsonObject(json, "conditions").entrySet()) {
					if (!entry.getValue().isJsonObject()) throw new Exception("Conditions are not in the JsonObject format");
					DataCondition condition = parseCondition(entry.getValue().getAsJsonObject(), entry.getKey());
					if (type == null || condition == null) throw new Exception("condition [type="+type + ", condition="+ condition +"] is not valid for Entity " + name);
	...
	private DataCondition parseCondition(JsonObject json, String name) throws Exception {
		...
		if (GsonHelper.isValidNode(json, "type")) type = DataType.of(GsonHelper.getAsString(json, "type"));
		if (GsonHelper.isValidNode(json, "target") && type != null) explorer = new NBTExplorer(GsonHelper.getAsString(json, "target"), type);
		if (GsonHelper.isValidNode(json, "operation")) {
			...
			if (operationString.equals("%")) {
				...
				if (GsonHelper.isValidNode(json, "value") && type != null) value = type.readFromJson(json.get("value"));
			}
		}
		if (!GsonHelper.isValidNode(json, "mode")) throw new Exception("Condition \"" + name + "\" does not have \"mode\" value");
		String mode = GsonHelper.getAsString(json, "mode");
		if (!(type == null || explorer == null || operation == null || value == null)) {
			if (mode.equals("user_nbt")) return new UserDataCondition(explorer, value, operation);
			else if (mode.equals("entity_nbt")) return new EntityDataCondition(explorer, value, operation);
			else if (mode.equals("world_nbt") || mode.equals("level_nbt")) return new WorldDataCondition(explorer, value, operation);
			else if (mode.equals("compare_nbt")) return new CompareDataCondition(explorer, new NBTExplorer((String) value, type), operation);
			else throw new Exception("\"" + mode + "\" for condition \"" + name + "\"  is not a valid condition mode");
		} else if (GsonHelper.isValidNode(json, "conditions")) {
			LogicalOperation gate = LogicalOperation.fromName(mode);
			List<DataCondition> subConditions = Lists.newArrayList();
			if (gate == null) throw new Exception("\"" + GsonHelper.getAsString(json, "mode") + "\" for condition \"" + name + "\" is not a valid logical operator");
			JsonArray array = GsonHelper.getAsJsonArray(json, "conditions");
			for (int i = 0; i < array.size(); i++) {
				try {
					subConditions.add(parseCondition((JsonObject) array.get(i), name+"_"+i));
				} catch (Exception e) {
		...
		} else throw new Exception("\"" + mode + "\" for condition \"" + name + "\" is not a valid mode");
	}
